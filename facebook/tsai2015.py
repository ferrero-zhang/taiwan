...
MONGOD_PORT = 27017
MONGOD_DB = 'taiwan'
MONGOD_COLLECTION = 'fb_status_tsaiingwen'
...
class Spider(object):
    ...
    def spider_website(self, keyword):
        ...
        if history2015:
            history2015.click()
        more = self.client.find_elements_by_xpath('/html/body/div/div/div[2]/div/div/div/div[2]/a')
        while more:
    ...
    for keyword in keywords:
        ...
        try:
            logging.basicConfig(level=logging.INFO,
                ...
                filename='facebook.log',
                filemode='a')
            logging.info(' start spider 2015Y tsai:' + str(ts2HMS_1(time.time())) )
            s.spider_website(keyword)
            logging.info(' end spider 2015Y tsai:' + str(ts2HMS_1(time.time())) )
