...
def shingle_news(start_cal, end_cal):
    ...
    news1 =  db[MONGOD_COLLECTION].find({'created_at':{'$gte':HMS2ts(start_cal),'$lte':HMS2ts(end_cal)}})
    news2 =  db[MONGOD_COLLECTION].find({'created_at':{'$gte':HMS2ts(start_cal),'$lte':HMS2ts(end_cal)}})
    ...
    for i in news1:
        ...
        ids1.append(i['id_tweet'])
        times1.append(i['created_at'])
        try:
    ...
    for j in news2:
        summary2.append(j['text'])
        ids2.append(j['id_tweet'])
        times2.append(j['created_at'])
        try:
                    ...
                    if(float(times1[count])>float(times2[k])):
                        ...
                        try:
                            ...
                            if isinstance(candidate,list):
                                ...
                                if keyword1[count] not in candidates:
                        ...
                        try:
                            ...
                            if isinstance(candidate,list):
                                for i in range(0,len(candidate)):
                                    candidates.append(candidate)
                                if keyword2[k] not in candidates:
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.INFO,
                ...
                filename='abnormal.log',
                filemode='a')
    logging.info('start cal shingle_twitterl :'+ str(ts2HMS_1(time.time()))  )
    shingle_news(start_cal, end_cal)
    logging.info('end cal shingle_twitterl :'+ str(ts2HMS_1(time.time()))  )
    #shingle_news()
    end_time = time.time()
    ca_time = end_time - start_time
    logging.info('total cal_time shingle_twitter :'+ str(ca_time)  )
    print "shingle_twitter cal_time:"+str(ca_time)
